...
# 每个预设: 16字符HEX字符串列表, 格式 FFFFIIII (4字节频率 + 4字节强度)
# 每条代表100ms
PRESETS = {
    "呼吸": [
...
def get_random_for_second(second: int):
    """根据秒数随机返回推荐波形 (name, data)"""
...
def scale_waveform(data: list, intensity: int) -> list:
    """缩放波形强度到指定值 (0-200)"""
    if not data:
        return data
    max_val = 0
    for entry in data:
        for j in range(4):
            ...
            if pos + 2 <= len(entry):
                try:
                    max_val = max(max_val, int(entry[pos:pos + 2], 16))
                except ValueError:
                    pass
    if max_val == 0:
        return data
    scale = intensity / max_val
    result = []
    for entry in data:
        if len(entry) >= 16:
            freq = entry[:8]
            ...
            for j in range(4):
                ...
                try:
                    ...
                    new_inten += f"{min(200, int(val * scale)):02X}"
                except ValueError:
                    new_inten += entry[pos:pos + 2]
            result.append(freq + new_inten)
        else:
            result.append(entry)
    return result


def loop_waveform(data: list, target_count: int) -> list:
    """循环波形数据到目标长度"""
    if not data:
        return data
    result = []
